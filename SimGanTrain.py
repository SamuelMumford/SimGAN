...
##SEE the SimFanTrainCL.py FOR A FULL COMMENTED VERSION OF THIS CODE
...
l1 = len(list(dataset))
l2 = len(list(dataset_mask))
ds = iter(dataset)
dsm = iter(dataset_mask)
...
def NSL(y_true, y_pred):
    a = -y_true*tf.math.log(tf.nn.softmax(y_pred))
...
#        Q, q = combined_model.predict_on_batch(face_img_batch)
#        print(NSL(y_real, q))
#        print(self_regularisation_loss(face_img_batch, Q))
        loss = combined_model.train_on_batch(face_img_batch, y=[face_img_batch, y_real])
