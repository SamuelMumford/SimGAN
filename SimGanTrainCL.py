...
#Name of the trained model, used for saving images and weights
name = 'gen5.h5'
...
#Delete pictures currently stored in the checkpoint folder
folder = basePath
...
def discriminator_model(width = 55, height = 35, channels = 1):
    input_layer = Input(shape=(height, width, channels))
    #Define a series of convolutions and max-pooling, eventually leading to a 
    #2-element output
    ...
    #BatchNorm is used to avoid exploding gradients
    x = BatchNormalization()(x)
    x = Flatten()(x)
...
#Set up the refiner or generator model
...
#Set up combined model for GAN training
...
for i in range(nb_steps):
    # train the refiner
    for p in range(k_d):
        # sample a mini-batch of artifical masked faces and masked images
        ...
        #Combine the real and generated images
    ...
    #Train the generator
    for q in range(k_g):
        # sample a mini-batch of bare faces
    ...
    #Display updates on training every log_interval
    ...
    #Save the model weights every save_interval    
